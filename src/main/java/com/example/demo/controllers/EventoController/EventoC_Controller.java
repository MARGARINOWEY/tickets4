	...
	public String eventoCR(Model model,HttpServletRequest request,@PathVariable("id_sector")Long id_sector,@RequestParam("correo")String correo) {
		...
		if (usuarioService.RecuperarUsuario(correo, "C8") != null) {
			...
			if (id_compra != 0) {
				...
				emailService.enviarMensajeRegistro(usuario.getCorreo(), "Reserva: "+sector.getEvento().getDesc_evento(), compra.getMonto_pagar(), sector.getEvento().getDesc_evento(),"CompraC4Email/"+compra.getId_compra(),sector.getDesc_sector(),t1_x[4]);
